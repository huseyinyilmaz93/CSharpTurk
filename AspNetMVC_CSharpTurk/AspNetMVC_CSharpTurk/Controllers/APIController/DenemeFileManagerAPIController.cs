...
namespace AspNetMVC_CSharpTurk.Controllers.APIController
{
    ...
    public class DenemeFilemanagerController : Controller
    {
        public string SetRoot(string userName)
        {
            if (!Directory.Exists(Server.MapPath(string.Concat(this.RootPath, "/", userName))))
                this.AddFolder("~/Files/Uploads/", userName);
            if (!userName.Equals("admin")) this.RootPath = string.Concat(this.RootPath, "/", userName);
        ...
        /// </summary>
        private string RootPath = "~/Files/Uploads"; // Root directory for all file uploads [string]
        ...
        /// </summary>
        private string IconDirectory = "~/Content/Images"; // Icon directory for filemanager. [string]
        ...
        /// 
        [HttpGet]
        public ActionResult Transaction(string mode, string path = null)
        {
            if (path == null) path = this.RootPath;
            else path = string.Concat("~", path);
            ...
            try
            {
                switch (mode)
                {
                    ...
                    case "getfolder":
                        return Content(GetFolderInfo(path));
                    case "move":
        ...
        [HttpGet]
        public ActionResult Tran(string mode, string path = null)
        {
            StringBuilder b = new StringBuilder();
            b.AppendLine("denemee");
